...
public abstract class Formica {
	...
	public Formica(int progId, Ambiente ambiente) {
	...
	public void simula(int passo) {
		if (ciboCaricato()) {
			...
			if (nelFormicaio()) {
				...
				this.getFormicaio().immagazzinaCaricoDi(this);
			} else {
				/* cibo gia' caricato: redireziona per rientrare verso il formicaio */
				final Direzione versoFormicaio = new Direzione(this.getPosizione(),this.getFormicaio().getPosizione());
				...
				this.getAmbiente().incrementaFerormone(this.getPosizione());
				eseguiSpostamento();
			}
		} else {
			/* ancora senza cibo: continua a cercare */
			final Cibo cibo = this.getAmbiente().getCibo(this.getPosizione());
			if (cibo!=null) {
	...
	private void raccogli(final Cibo cibo) {
		this.setCarico(this.getAmbiente().remove(cibo));
