...
public class Statistiche {
	...
	public Map<Object, Integer> raccoltoPerFormica(Set<Cibo> raccolto) {
		...
		Map<Object, Integer> formica2raccolto = new HashMap<>();

		for (Cibo cibo : raccolto) {
			Object key = cibo.getRaccoglitrice();
			Integer value = formica2raccolto.get(key);

			if(value==null)
				value = 0;
			value++;
			formica2raccolto.put(key, value);
		}
	...
	public Map<Class<?>, Set<Cibo>> raccoltoPerTipoDiFormica(Set<Cibo> raccolto) {
		...
		Map<Class<?>, Set<Cibo>> tipoFormica2raccolto = new HashMap<>();
		
		for (Cibo cibo : raccolto) {
			Class<?> key = cibo.getRaccoglitrice().getClass();
			Set<Cibo> value = tipoFormica2raccolto.get(key);
			
			if(value==null)
				value = new HashSet<>();
			value.add(cibo);
			
			tipoFormica2raccolto.put(key, value);
		}
	...
	public List<Class<?>> ordinaStrategieDiRaccolta(final Map<Class<?>, Set<Cibo>> c2c) {
		...
		TreeSet<Class<?>> strategieOrdinate = new TreeSet<>(new comparatoreStrategieDecrescente(c2c));
		strategieOrdinate.addAll(c2c.keySet());
		
		return  new ArrayList<>(strategieOrdinate);
	}
